    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <section className="pt-24 pb-20 text-center">
        <div className={ready ? 'anim-fade-up' : 'opacity-0'}>
          <div className="inline-flex items-center gap-2 rounded-full px-4 py-1.5 text-xs mb-8 bg-indigo-500/10 border border-indigo-500/20 text-indigo-300">
        ...
        <h1 className={`text-4xl sm:text-5xl lg:text-6xl font-bold mb-5 leading-tight tracking-tight ${ready ? 'anim-fade-up del-1' : 'opacity-0'}`}>
          Expose corruption.<br /><span className="gradient-text">Stay anonymous.</span>
        </h1>

        <p className={`text-base sm:text-lg text-zinc-400 max-w-xl mx-auto mb-10 leading-relaxed ${ready ? 'anim-fade-up del-2' : 'opacity-0'}`}>
          Submit evidence securely using <span className="text-white font-medium">Algorand</span>,{' '}
          <span className="text-white font-medium">IPFS</span>, and{' '}
          <span className="text-white font-medium">military-grade encryption</span>.
        </p>
        ...
        <div className={`flex flex-col sm:flex-row items-center justify-center gap-3 ${ready ? 'anim-fade-up del-3' : 'opacity-0'}`}>
          ...
          <Link to="/how-it-works" className="px-8 py-3.5 rounded-xl font-medium text-sm text-zinc-300 hover:text-white bg-zinc-800/50 border border-zinc-700 hover:border-zinc-600 transition-all">
            How It Works
          </Link>
        </div>

        {health && (
          <div className="mt-10 inline-flex items-center gap-3 text-xs text-zinc-500 anim-fade-in del-4">
            <span className={`w-1.5 h-1.5 rounded-full ${health.algorand_connected ? 'bg-emerald-400' : 'bg-red-400'}`} />
        ...
        {FEATURES.map(({ icon: I, title, desc }) => (
          <div key={title} className="card p-6 hover:border-zinc-700 transition-colors">
            <div className="flex items-start gap-4">
              <div className="w-10 h-10 rounded-lg bg-indigo-500/10 border border-indigo-500/20 grid place-items-center shrink-0">
                <I className="w-5 h-5 text-indigo-400" />
              </div>
              <div>
      ...
      <section className="pb-24">
        <div className="text-center mb-12">
          ...
          <p className="text-sm text-zinc-400">From anonymous submission to immutable record in four steps</p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {STEPS.map(({ n, icon: I, t, d }) => (
            <div key={n} className="card p-5 hover:border-zinc-700 transition-colors">
              <div className="text-xs font-mono text-indigo-400 mb-3">{n}</div>
              <div className="w-9 h-9 rounded-lg bg-zinc-800 grid place-items-center mb-3"><I className="w-4 h-4 text-zinc-300" /></div>
      ...
      <section className="pb-20">
        <div className="card p-10 sm:p-14 text-center">
          <h3 className="text-xl sm:text-2xl font-bold mb-3">Ready to expose the truth?</h3>
          <p className="text-sm text-zinc-400 mb-8 max-w-md mx-auto">Your identity is protected by cryptography and decentralized infrastructure.</p>
      ...
      {/* ── Footer ── */}
      <footer className="pb-10 text-center border-t border-zinc-800 pt-8">
        <div className="flex items-center justify-center gap-2 text-xs text-zinc-500 flex-wrap">
          ...
          <span className="font-mono text-zinc-400">App ID 755784943</span><span>·</span>
          <a href="https://testnet.explorer.perawallet.app/application/755784943" target="_blank" rel="noopener noreferrer"
            className="inline-flex items-center gap-1 text-indigo-400 hover:text-indigo-300">Explorer <ExternalLink className="w-3 h-3" /></a>
        </div>
        <p className="text-xs text-zinc-600 mt-2">WhistleChain RIFT 2026 — Trust math, not promises.</p>
      </footer>
    </div>
  );
}
